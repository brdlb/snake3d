...
export type RoomAction = 'initial' | 'resume' | 'restart' | 'next';
...
export class NetworkManager {
  ...
  async requestRoom(action:RoomAction, contextSeed?:number):Promise<RoomData> { const room=await this.api<RoomData>('/api/v1/matches',{method:'POST',body:JSON.stringify({action,contextSeed})}); this.roomSeed=room.seed; this.openRoomSocket(room.seed); return room; }
  ...
  private openRoomSocket(seed:number) { this.socket?.close(); const url=new URL(`${this.apiBase}/api/v1/rooms/${seed}/socket`);url.protocol=url.protocol==='https:'?'wss:':'ws:'; this.socket=new WebSocket(url); this.socket.onopen=()=>{this.retries=0;this.emit('roomSocketConnected');}; this.socket.onmessage=(event)=>{try{const msg=JSON.parse(event.data);this.emit(msg.type,msg.payload??msg);}catch{}}; this.socket.onclose=()=>this.scheduleReconnect(); this.socket.onerror=()=>this.socket?.close(); }
  private scheduleReconnect(){if(this.roomSeed===null||(typeof navigator!=='undefined'&&!navigator.onLine)||this.retries>=5)return;const delay=Math.min(1000*2**this.retries++,10000);window.setTimeout(()=>this.roomSeed!==null&&this.openRoomSocket(this.roomSeed),delay);}
  private reconnectRoom(){if(this.roomSeed!==null)this.openRoomSocket(this.roomSeed);}
