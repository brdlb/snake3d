...
export class NetworkManager {
  ...
  send(event:string,data?:any) { if(event==='room:join'){void this.requestRoom(data?.action ?? 'initial',data?.contextSeed);return;} if(event==='game:over'){void this.submitGame(data);return;} if(this.socket?.readyState===WebSocket.OPEN)this.socket.send(JSON.stringify({v:1,type:event,id:crypto.randomUUID(),payload:data})); }
  async requestRoom(action:RoomAction, contextSeed?:number):Promise<RoomData> { const room=await this.api<RoomData>('/api/v1/matches',{method:'POST',body:JSON.stringify(action==='join'||action==='spectate'?{action,seed:contextSeed}:{action,contextSeed})}); this.roomSeed=room.seed; this.roomIsSpectator=action==='spectate'; this.openRoomSocket(room.seed,this.roomIsSpectator); return room; }
  async submitGame(data:any) { try { const result=await this.api<{saved:boolean;message:string}>(`/api/v1/rooms/${data.seed}/games`,{method:'POST',body:JSON.stringify({submissionId:data.submissionId ?? crypto.randomUUID(),replay:data.replay})});this.emit('game:result',result);return result; } catch(error) { const result={saved:false,message:error instanceof Error?error.message:'Failed to save game'};this.emit('game:result',result);throw error; } }
  ...
  private openRoomSocket(seed:number, spectating=false) { const previous=this.socket; if(this.heartbeat!==null)window.clearInterval(this.heartbeat); this.heartbeat=null; const url=new URL(`${this.apiBase}/api/v1/rooms/${seed}/socket`);if(spectating)url.searchParams.set('spectator','1');url.protocol=url.protocol==='https:'?'wss:':'ws:'; const socket=this.socket=new WebSocket(url); previous?.close(); socket.onopen=()=>{if(this.socket!==socket)return;this.retries=0;socket.send(JSON.stringify({v:2,type:'room.resync'}));this.heartbeat=window.setInterval(()=>socket.readyState===WebSocket.OPEN&&socket.send(JSON.stringify({v:2,type:'ping'})),15000);this.emit('roomSocketConnected');}; socket.onmessage=(event)=>{if(this.socket!==socket)return;try{const msg=JSON.parse(event.data);this.emit(msg.type,msg.payload??msg);}catch{}}; socket.onclose=()=>{if(this.socket!==socket)return;if(this.heartbeat!==null)window.clearInterval(this.heartbeat);this.heartbeat=null;this.scheduleReconnect();}; socket.onerror=()=>socket.close(); }
  sendDirection(direction:{x:number;y:number;z:number}) { if(this.socket?.readyState===WebSocket.OPEN)this.socket.send(JSON.stringify({v:2,type:'player.input',payload:{timestamp:Date.now(),action:{type:'direction',direction}}})); }
  private scheduleReconnect(){if(this.roomSeed===null||(typeof navigator!=='undefined'&&!navigator.onLine)||this.retries>=5)return;const delay=Math.min(1000*2**this.retries++,10000);window.setTimeout(()=>this.roomSeed!==null&&this.openRoomSocket(this.roomSeed,this.roomIsSpectator),delay);}
  private reconnectRoom(){if(this.roomSeed!==null)this.openRoomSocket(this.roomSeed,this.roomIsSpectator);}
