...
type RoomAction = 'initial' | 'resume' | 'restart' | 'next';
type RoomData = { seed: number; phantoms: unknown[]; playerSpawnIndex: number };
...
export function replayReplacementOrder(replaySaved: boolean, evictRoomReplay: boolean) {
  ...
  return [...(evictRoomReplay ? ['deleteRoomMinimum'] : []), 'deletePlayerReplay', 'insertReplay'] as const;
}
...
export default { async fetch(request, env): Promise<Response> { const requestId=crypto.randomUUID(), url=new URL(request.url), path=url.pathname;
  ...
  if(path==='/api/v1/matches'&&request.method==='POST') { const data=await body(request,requestId); if(data instanceof Response)return data; const p=data as {action?:unknown;contextSeed?:unknown}; if(typeof p.action!=='string'||!roomActions.has(p.action as RoomAction)||p.contextSeed!==undefined&&(!Number.isSafeInteger(p.contextSeed)))return fail('INVALID_ROOM_ACTION',400,requestId); return env.ROOMS.get(env.ROOMS.idFromName(`user:${user.id}`)).fetch('https://room/assign',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:p.action,contextSeed:p.contextSeed,user})}); }
  ...
  const socket=path.match(/^\/api\/v1\/rooms\/(-?\d+)\/socket$/); if(socket&&request.headers.get('upgrade')==='websocket') { const assigned=await env.DB.prepare('SELECT 1 FROM room_assignments WHERE user_id=? AND room_seed=?').bind(user.id,Number(socket[1])).first(); if(!assigned)return fail('ROOM_CONTEXT_MISMATCH',409,requestId); return env.ROOMS.get(env.ROOMS.idFromName(socket[1])).fetch('https://room/socket',{headers:{upgrade:'websocket','x-user':JSON.stringify(user),'x-seed':socket[1]}}); }
...
export class RoomDurableObject {
  ...
  async fetch(request: Request): Promise<Response> { const path=new URL(request.url).pathname; if(path==='/socket') return this.socket(request); const data=await request.json<any>(); return path==='/assign' ? this.assign(data) : path==='/submit' ? this.submit(data) : new Response('Not found',{status:404}); }
