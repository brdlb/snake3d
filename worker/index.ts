import { addPlayer, chooseLowestPhantom, createSimulation, validInput, validOrientation, type Axis, type SimPlayer, type SimulationState } from '../shared/simulation';
import type { DeathInput, DirectionInput, RealtimeClientMessage, StateInput } from '../shared/realtime';
...
function sequence(value: unknown) { return typeof value==='number' && Number.isSafeInteger(value) && value >= 0; }
function snakeState(value: unknown): value is Omit<StateInput,'type'|'seq'|'step'|'reason'> { if (!value || typeof value !== 'object') return false; const state=value as Record<string,unknown>; return Array.isArray(state.segments) && state.segments.length > 0 && state.segments.length <= 10000 && state.segments.every(segment => position(segment) && Object.values(segment as Record<string,number>).every(Number.isInteger)) && validOrientation(state.direction,state.up) && typeof state.score === 'number' && Number.isSafeInteger(state.score) && state.score >= 0 && typeof state.speed === 'number' && Number.isFinite(state.speed) && state.speed >= 60; }
function stateInput(value: unknown): value is StateInput { if(!snakeState(value))return false; const action=value as Record<string,unknown>; return action.type==='state'&&sequence(action.seq)&&sequence(action.step)&&(action.reason==='food'||action.reason==='speed'||action.reason==='reconnect'||action.reason==='spawn'); }
function deathInput(value: unknown): value is DeathInput { if(!snakeState(value))return false; const action=value as Record<string,unknown>; return action.type==='death'&&sequence(action.seq)&&sequence(action.step)&&typeof action.reason==='string'; }
function directionInput(value: unknown): value is DirectionInput { if(!value||typeof value!=='object')return false; const action=value as Record<string,unknown>; return action.type==='direction'&&sequence(action.seq)&&sequence(action.step)&&position(action.head)&&Object.values(action.head as Record<string,number>).every(Number.isInteger)&&validOrientation(action.direction,action.up); }
...
export function chooseSpawn(phantoms: Array<{ score?: number; elo?: number; startParams?: { spawnIndex?: number } }>, previousSpawnIndex?: number) {
...
export class RoomDurableObject {
  ...
  private async persist(state: SimulationState) { await this.ctx.storage.put('simulation',state); const deadlines=Object.values(state.players).map(player=>player.disconnectedAt).filter((value): value is number => Number.isFinite(value)); const next=Math.min(...deadlines); if(Number.isFinite(next))await this.ctx.storage.setAlarm(next); else await this.ctx.storage.deleteAlarm(); }
  ...
  private trackState(state: SimulationState, player: SimPlayer) { const path=state.trajectories?.[player.id]??[]; path.push({...player.segments[0]}); if(path.length>10000)path.splice(0,path.length-10000); (state.trajectories??={})[player.id]=path; }
  async alarm() { const state=this.simulation??await this.ctx.storage.get<SimulationState>('simulation'); if(!state)return; this.simulation=state; this.cleanupDisconnected(state,Date.now()); await this.persist(state); }
  ...
  private async restorePhantoms(state: SimulationState) {
    ...
    const assignments = await this.env.DB.prepare('SELECT spawn_index FROM room_assignments WHERE room_seed=?').bind(state.seed).all<{spawn_index:number}>();
  ...
  private async loadState(seed: number) { this.simulation ??= await this.ctx.storage.get<SimulationState>('simulation') ?? createSimulation(seed); return this.simulation; }
  private async prepareState(seed: number) { const state=await this.loadState(seed); await this.restorePhantoms(state); return state; }
  ...
  private async liveState(seed: number) { const needsPreparation=this.simulation===null||this.restartUserId!==null; const state=await this.loadState(seed); if(needsPreparation)await this.restorePhantoms(state); return state; }
  ...
  private async activeReplays(seed:number, excludePlayerId?:string) {
    const replays=await this.env.DB.prepare(ROOM_REPLAYS_QUERY).bind(seed).all<{payload_json:string}>();
    if (!excludePlayerId) return replays;
    return { ...replays, results: replays.results.filter(row => JSON.parse(row.payload_json).playerId !== excludePlayerId) };
  }
  private async roomData(seed:number,user:User,excludeAssignedSpawn=false):Promise<RoomData> {
    const replays=await this.activeReplays(seed, excludeAssignedSpawn ? user.id : undefined);
    const assignment=await this.env.DB.prepare('SELECT spawn_index FROM room_assignments WHERE user_id=? AND room_seed=?').bind(user.id,seed).first<{spawn_index:number}>();
    const playerSpawnIndex=assignment?.spawn_index??0;
    return {
      seed,
      phantoms: replays.results
        .map(r=>JSON.parse(r.payload_json))
        .filter(replay => !excludeAssignedSpawn || replay.startParams?.spawnIndex !== playerSpawnIndex),
      playerSpawnIndex,
    };
  }
  ...
  private async saveTerminal(seed:number, terminal:{player:SimPlayer;deathPosition:unknown;reason:string;trajectory:unknown[]}) { if(terminal.player.phantom)return null; const userRow=await this.env.DB.prepare('SELECT * FROM users WHERE id=?').bind(terminal.player.id).first<Record<string,unknown>>(); if(!userRow)return null; const user=toUser(userRow),submissionId=`terminal:${seed}:${user.id}:${terminal.player.instanceId??'none'}`,old=await this.env.DB.prepare('SELECT result_json FROM game_submissions WHERE id=? AND user_id=?').bind(submissionId,user.id).first<{result_json:string}>(); if(old)return JSON.parse(old.result_json); const id=crypto.randomUUID(),time=now(),score=terminal.player.score,stored={id,playerId:user.id,playerName:user.username,finalScore:score,deathPosition:terminal.deathPosition,trajectoryLog:terminal.trajectory.map(position=>({position,direction:terminal.player.direction})),timestamp:Date.now(),elo:user.elo,terminalReason:terminal.reason}; const worst=await this.env.DB.prepare('SELECT id,score FROM replays WHERE room_seed=? ORDER BY score ASC,created_at ASC LIMIT 1').bind(seed).first<{id:string;score:number}>(); const count=await this.env.DB.prepare('SELECT COUNT(*) AS count FROM replays WHERE room_seed=?').bind(seed).first<{count:number}>(); const replaySaved=Number(count?.count??0)<3||score>Number(worst?.score??Number.MAX_SAFE_INTEGER); const result={saved:true,replaySaved,replayId:replaySaved?id:undefined,message:replaySaved?'Game and replay saved':'Game saved; replay did not beat the room minimum'}; const statements=[this.env.DB.prepare('UPDATE rooms SET total_games_played=total_games_played+1,updated_at=? WHERE seed=?').bind(time,seed),this.env.DB.prepare('UPDATE users SET games_played=games_played+1,total_score=total_score+?,high_score=CASE WHEN ?>high_score THEN ? ELSE high_score END,high_score_seed=CASE WHEN ?>high_score THEN ? ELSE high_score_seed END,high_score_replay_id=CASE WHEN ?>high_score THEN ? ELSE high_score_replay_id END,high_score_date=CASE WHEN ?>high_score THEN ? ELSE high_score_date END,elo=CASE WHEN ?>high_score THEN ? ELSE elo END,last_seen=? WHERE id=?').bind(score,score,score,score,seed,score,id,score,time,score,score,time,user.id),this.env.DB.prepare('INSERT INTO game_submissions(id,user_id,room_seed,replay_id,result_json,created_at) VALUES(?,?,?,?,?,?)').bind(submissionId,user.id,seed,replaySaved?id:null,JSON.stringify(result),time)]; if(replaySaved){for(const operation of replayReplacementOrder(true,Boolean(worst&&Number(count?.count)>=3)))statements.unshift(operation==='deleteRoomMinimum'?this.env.DB.prepare('DELETE FROM replays WHERE id=?').bind(worst!.id):this.env.DB.prepare('INSERT INTO replays(id,room_seed,user_id,score,created_at,payload_json) VALUES(?,?,?,?,?,?)').bind(id,seed,user.id,score,time,JSON.stringify(stored)));} await this.env.DB.batch(statements); return result; }
   private async socket(request:Request) { if(request.headers.get('upgrade')!=='websocket')return new Response('Upgrade required',{status:426}); const pair=new WebSocketPair(),[client,server]=Object.values(pair),user=JSON.parse(request.headers.get('x-user')||'{}') as User,seed=Number(request.headers.get('x-seed')),spectator=request.headers.get('x-spectator')==='1',state=await this.liveState(seed); let player=Object.values(state.players).find(candidate=>candidate.id===user.id&&!candidate.phantom); let playerJoined=false; const connectionId=crypto.randomUUID(); if(!spectator){ const assigned=await this.env.DB.prepare('SELECT spawn_index FROM room_assignments WHERE room_seed=? AND user_id=?').bind(seed,user.id).first<{spawn_index:number}>(); const spawnIndex=assigned?.spawn_index??0; if(!player||!player.alive){if(player)delete state.players[player.entityId];player=addPlayer(state,user.id,user.username,Date.now(),{spawnIndex,instanceId:connectionId});playerJoined=true;} else { player.instanceId=connectionId; player.disconnectedAt=undefined; for(const socket of this.ctx.getWebSockets())try{const attachment=socket.deserializeAttachment() as {userId?:string}|null;if(attachment?.userId===user.id)socket.close(4001,'Replaced by a newer connection');}catch{} } await this.persist(state);} this.ctx.acceptWebSocket(server); server.serializeAttachment({userId:user.id,entityId:player?.entityId,seed,user,spectator,instanceId:player?.instanceId}); server.send(JSON.stringify({v:2,type:'room.state',payload:this.snapshot(state)})); if(playerJoined)this.broadcast({v:2,type:'player.joined',payload:{user:{id:user.id,username:user.username},timestamp:Date.now(),action:{type:'spawn',entityId:player!.entityId,position:player!.segments[0],direction:player!.direction,up:player!.up}}}); this.presence(); return new Response(null,{status:101,webSocket:client}); }
      async webSocketMessage(ws:WebSocket,message:string|ArrayBuffer) { try { const event=JSON.parse(String(message)) as RealtimeClientMessage,attachment=ws.deserializeAttachment() as {userId:string;entityId?:string;seed:number;user:User;spectator?:boolean;instanceId?:string}; if(event?.v!==2)throw new Error(); if(event.type==='ping'){ws.send(JSON.stringify({v:2,type:'pong'}));return;} const state=await this.liveState(attachment.seed); if(event.type==='room.resync'){ws.send(JSON.stringify({v:2,type:'room.state',payload:this.snapshot(state)}));return;} if(attachment.spectator)throw new Error(); const player=attachment.entityId?state.players[attachment.entityId]:undefined,action='payload' in event?event.payload.action:undefined; if(!player||player.instanceId!==attachment.instanceId){ws.send(JSON.stringify({v:2,type:'error',payload:{code:'STALE_CONNECTION'}}));return;} if(!player.alive){ws.send(JSON.stringify({v:2,type:'error',payload:{code:'PLAYER_NOT_ALIVE'}}));return;} if(event.type==='player.state'&&stateInput(action)){if(action.seq<= (player.lastStateSeq??-1))return;player.segments=action.segments;player.direction=action.direction;player.up=action.up;player.score=action.score;player.speed=action.speed;player.lastStateSeq=action.seq;state.tick++;this.trackState(state,player);await this.persist(state);this.broadcast({v:2,type:'player.state',payload:{entityId:player.entityId,segments:player.segments,direction:player.direction,up:player.up,score:player.score,speed:player.speed,seq:action.seq,step:action.step,reason:action.reason,serverTime:Date.now()}},ws);return;} if(event.type==='player.died'&&deathInput(action)){if(action.seq<= (player.lastStateSeq??-1))return;player.segments=action.segments;player.direction=action.direction;player.up=action.up;player.score=action.score;player.speed=action.speed;player.lastStateSeq=action.seq;player.alive=false;state.tick++;this.trackState(state,player);const deathPosition=player.segments[0],trajectory=state.trajectories?.[player.id]??[];await this.ctx.storage.put(`terminal:${player.id}`,{player,deathPosition,reason:action.reason,trajectory,tick:state.tick,createdAt:Date.now()});await this.persist(state);const result=await this.saveTerminal(state.seed,{player,deathPosition,reason:action.reason,trajectory});this.broadcast({v:2,type:'player.died',payload:{player,position:deathPosition,reason:action.reason,serverTime:Date.now()}},ws);if(result)this.sendToUser(player.id,{v:2,type:'game.saved',payload:result});return;} if(event.type!=='player.directionChanged'||!directionInput(action)||action.seq<= (player.lastInputSeq??-1)||!validInput(player.direction,action.direction,player.up,action.up)){ws.send(JSON.stringify({v:2,type:'error',payload:{code:'INVALID_MESSAGE'}}));return;} player.direction=action.direction;player.up=action.up;player.lastInputSeq=action.seq;await this.persist(state);this.broadcast({v:2,type:'player.directionChanged',payload:{entityId:player.entityId,seq:action.seq,step:action.step,head:action.head,direction:player.direction,up:player.up,speed:player.speed,serverTime:Date.now()}},ws); } catch { ws.send(JSON.stringify({v:2,type:'error',payload:{code:'INVALID_MESSAGE'}})); } }
  ...
  private presence() { this.broadcast({v:2,type:'presence.updated',payload:{count:this.ctx.getWebSockets().length}}); }
  ...
  private broadcast(message:unknown, except?:WebSocket) { const text=JSON.stringify(message); for(const socket of this.ctx.getWebSockets())try{if(socket!==except)socket.send(text);}catch{socket.close();} }
}
