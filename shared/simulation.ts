...
export type SimulationState = {
  ...
  trajectories?: Record<string, ReplayTrajectory>;
};
export type DeathReason = 'bounds' | 'body' | 'head-to-head';
export type FoodChange = {
  removed: Food;
  added: Food;
  entityId?: string;
  score?: number;
...
export type SimulationDelta = {
  changed: SimPlayer[];
  food: FoodChange[];
  deaths: Array<{ player: SimPlayer; position: Axis; reason: DeathReason }>;
  tick: number;
};
...
const inBounds = (p: Axis) =>
  p.x >= 0 && p.x <= WORLD_SIZE && p.y >= 0 && p.y <= WORLD_SIZE && p.z >= 0 && p.z <= WORLD_SIZE;
...
export function foodEffect(kind: FoodKind) {
  return kind === 'green'
    ? { score: 5, growth: 3, speed: 50 }
    : kind === 'blue'
      ? { score: 15, growth: 5, speed: 10 }
...
export function safeSpawn(
  ...
  const start = Number.isInteger(requestedIndex)
    ? ((requestedIndex! % spawnPoints.length) + spawnPoints.length) % spawnPoints.length
    : 0;
...
export function addPlayer(
  ...
  const spawn = safeSpawn(state, undefined, options.spawnIndex);
  const direction = options.direction ?? spawn.direction,
    up = options.up ?? { x: 0, y: 1, z: 0 },
    head = options.startPosition ?? options.segments?.[0] ?? spawn.position,
    entityId = options.entityId ?? `${id}:${now}:${Object.keys(state.players).length}`;
...
export function validOrientation(direction: unknown, up: unknown): direction is Axis {
  if (!direction || !up || typeof direction !== 'object' || typeof up !== 'object') return false;
  const d = direction as Axis,
    u = up as Axis;
  const dLength = d.x ** 2 + d.y ** 2 + d.z ** 2,
    uLength = u.x ** 2 + u.y ** 2 + u.z ** 2;
  return (
    directions.some((v) => same(v, d)) &&
    directions.some((v) => same(v, u)) &&
    Math.abs(dLength - 1) < 1e-9 &&
    Math.abs(uLength - 1) < 1e-9 &&
    Math.abs(d.x * u.x + d.y * u.y + d.z * u.z) < 1e-9
  );
}
export function validInput(
  ...
  const v = next as Axis;
  return (
    directions.some((d) => same(d, v)) &&
    !(v.x === -current.x && v.y === -current.y && v.z === -current.z) &&
    (nextUp === undefined || validOrientation(v, nextUp))
  );
}
function respawn(state: SimulationState, index: number): FoodChange {
  ...
  for (let i = 0; i < 4096; i++) {
    ...
    if (!occupied.has(key(p)) && !state.food.some((f, n) => n !== index && same(f, p))) {
      state.food[index] = {
        ...p,
        kind: random() < 0.5 ? 'blue' : random() < 0.8 ? 'green' : 'pink',
      };
...
export function stepSimulationDelta(
  state: SimulationState,
  at: number,
  allowDeaths = true,
  ...
  const foodAt = new Map(
    due.map((p) => [p.id, state.food.findIndex((f) => same(f, heads.get(p.id)!))]),
  );
  const freed = new Set(
    due
      .filter((p) => p.growth === 0 && foodAt.get(p.id) === -1)
      .map((p) => key(p.segments[p.segments.length - 1])),
  );
  ...
  for (const head of heads.values())
    headCounts.set(key(head), (headCounts.get(key(head)) ?? 0) + 1);
  const changed: SimPlayer[] = [],
    food: FoodChange[] = [],
    deaths: SimulationDelta['deaths'] = [];
  for (const player of due) {
    const head = heads.get(player.id)!;
    let reason: DeathReason | undefined;
    ...
    if (foodIndex >= 0) {
      ...
      const replacement = respawn(state, foodIndex);
      food.push({
        ...replacement,
        ...
        length: player.segments.length + (player.growth > 0 ? 1 : 0),
      });
    }
    ...
    player.segments = [
      clone(head),
      ...player.segments.slice(0, keepTail ? player.segments.length : player.segments.length - 1),
    ];
    if (keepTail) player.segments.push(clone(tail));
    player.nextStepAt = at + 60000 / player.speed;
    changed.push(clone(player));
  }
  state.tick++;
  return { changed, food, deaths, tick: state.tick };
}
...
export function stepSimulation(state: SimulationState, now: number) {
  return advanceSimulation(state, now).length > 0;
}
export function chooseLowestPhantom(players: SimPlayer[]) {
  return players
    .filter((p) => p.phantom)
